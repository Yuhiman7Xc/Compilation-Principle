...
void get_wenfa(){
    ...
    string line, s, foo;
    int cnt = 0;
    while(getline(cin, line)){  //读取一行语法规则
        cnt ++;
        int len = line.size();
        int i = 0;
...
void get_ch(){
    fstream in;
    in.open("./input/grammar.txt", ios::in);
...
bool grammar_analysis(){
    get_wenfa();    //LL(1)语法分析
    get_first();    //输出FIRST集
    get_follow();   //输出FOLLOW集
    get_select();   //输出SELECT集
    get_pre_list(); //输出语法分析结果（分析表）

//输出分析过程
    ofstream out;
    out.open("./output/process.txt", ios::out);
