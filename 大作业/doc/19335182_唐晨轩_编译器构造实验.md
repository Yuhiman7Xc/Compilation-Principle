...
&emsp;后端大致思路为，将前端四元式用接口函数转换为后端待处理四元式的数据结构。接下来目标代码生成算法处理此序列得到目标汇编指令。其中，目标代码生成采用多寄存器分配方法，首先分配寄存器，处理操作数寻址方式，并生成对应的目标代码，然后生成运算部分的目标代码。处理了很多算法细节使算法更加健壮完备，生成的代码是基于 ARM 的汇编指令。
