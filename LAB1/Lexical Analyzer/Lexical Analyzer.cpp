...
// 以下为各种简单词法
bool isNum(char chr) {//判断数字 [0]
    return  chr >= '0' && chr <= '9';
}

bool isLetter(char chr) {//判断字符 [1]
    //由于变量名中可以出现下划线，所以这里对下划线也返回true,对于引入的文件，可能存在.，也加入判断
    return (chr >= 'a' && chr <= 'z') || (chr >= 'A' && chr <= 'Z') || (chr == '_') || (chr == '.');
}
//定界符 单个
char symbol[] = { '#','<','>','(',')','[',']','{','}',',',';','+','-','*','/','%','!','\'','"','=' };//20个 0~19 +[2]，即2~21
string twoSymbol[] = { "++","--","&&","||",">=","<=","==","!=",">>","<<",":=" };//11个 0~10+[22]，即22~32
...
void init() {
    ...
    //            }
            ...
            for (unsigned int i = 0; i < strlen(str);) {//遍历此行
...
int main() {
    freopen("demo.cpp", "r", stdin);//文件标准读入
    freopen("tokens.txt", "w", stdout);//文件标准输出
    init();
    return 0;
}
